...
class OptModelCVaR:
    ...
    def _build(self):
        ...
        for t in self.T:
            for s in self.S:
                ...
                # CHANGED: Budget constraint uses v_budget[t] instead of v_budget[t, s]
                self.cons.budget[t, s] = self.model.addConstr(
                    self.vars.bought[t, s] * self.data.price_scenarios[s, t] + 
                    self.vars.stored[t, s] * self.data.storage_cost <= self.vars.budget[t], 
        ...
        for t in list(range(1, len(self.data.demand_scenarios[0]))):
            for s in self.S:
                ...
                self.cons.balance[t, s] = self.model.addConstr(
                    self.vars.bought[t, s] + self.vars.stored[t-1, s] == 
                    self.data.demand_scenarios[s, t] - self.vars.unmet_demand[t, s] + self.vars.stored[t, s], 
                    name=f"balance_{t}_{s}")
            
            # CHANGED: Depreciation constraint - one per time period (not per scenario)
            # Use EXPECTED spending across scenarios for budget evolution
            expected_spending = gp.quicksum(
                self.probs[s] * (
                    self.vars.bought[t-1, s] * self.data.price_scenarios[s, t-1] + 
                    self.vars.stored[t-1, s] * self.data.storage_cost
                )
                for s in self.S
            )
        ...
        for s in self.S:
            self.cons.balance[0, s] = self.model.addConstr(
                ...
                self.data.demand_scenarios[s, 0] - self.vars.unmet_demand[0, s] + self.vars.stored[0, s], 
                name=f"balance_0_{s}")
        ...
        # CHANGED: Initial budget constraint - one for all scenarios
    ...
    def solve(self, verbose: bool = False):
        ...
        # Replicate it across scenarios for compatibility with existing plotting code
        budget_1d = np.array([v.budget[t].X for t in self.T])
        self.results.v_budget = np.tile(budget_1d.reshape(-1, 1), (1, len(self.S)))  # Replicate for all scenarios
        ...
        # CHANGED: Depreciation duals are now 1D (only time dimension)
